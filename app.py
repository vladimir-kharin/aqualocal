...
# Таблицу «имя клавиши -> скан-код» пакет keyboard строит один раз за процесс, по
# раскладке, активной в этот момент (_setup_name_tables делает ранний выход, если
# таблица уже собрана). Стартовали на русской — обратного апострофа в таблице нет,
# и хоткей ctrl+` не разобрать уже никогда, даже после переключения раскладки.
# Поэтому собираем таблицу сами, включив английскую раскладку только своему потоку:
# у остальных окон раскладка своя, их это не касается.
KLF_NOTELLSHELL = 0x0080
LAYOUT_EN = '00000409'      # US English


def setup_key_names():
    """Собрать таблицу имён клавиш по английской раскладке, не трогая пользовательскую."""
    ...
    try:
        ...
        english = user32.LoadKeyboardLayoutW(LAYOUT_EN, KLF_NOTELLSHELL)
        if not english:
            logging.warning("Английская раскладка недоступна — имена клавиш соберутся "
                            "по текущей, символьные хоткеи могут не разобраться.")
            return
        user32.ActivateKeyboardLayout(english, 0)
        keyboard._winkeyboard._setup_name_tables()
        logging.info("Таблица имён клавиш собрана по английской раскладке.")
    except Exception as e:
        logging.error(f"Не удалось собрать таблицу имён клавиш: {e}")
    finally:
        if previous:
            try:
                user32.ActivateKeyboardLayout(previous, 0)
            except Exception as e:
                logging.error(f"Не удалось вернуть раскладку своему потоку: {e}")


# Строго до первого обращения к keyboard: кто первым тронет таблицу, тот и
# зафиксирует раскладку, а слушатель клавиатуры стартует сам при первом is_pressed.
setup_key_names()
...
def hotkey_codes(name):
    ...
    Имена символьных клавиш ('`', ';', '[') keyboard резолвит через таблицу
    раскладки; за то, чтобы она была английской, отвечает setup_key_names().
    Скан-коды от раскладки не зависят, поэтому разбираем имя один раз и дальше
    проверяем нажатие по кодам. Повтор раз в 2 секунды оставлен страховкой на
    случай, если таблицу собрать не удалось: сам по себе он не поможет —
    keyboard собирает её единожды за процесс.
    """
    ...
    try:
        ...
        if now - _hotkey_warned_at.get(name, 0.0) > 60:
            ...
            logging.warning(f"Хоткей {name.upper()} не разобрать ({e}). Символьная клавиша "
                            f"есть только в латинской раскладке, а таблицу имён клавиш собрать "
                            f"по ней не вышло — поможет перезапуск при английской раскладке.")
        return None
...
def exit_action(icon, item):
    ...
    app_running = False
    # Значок убираем сами и прямо сейчас. icon.stop() лишь шлёт потоку трея
    # сообщение, а os._exit ниже убивает процесс раньше, чем тот успеет сказать
    # системе NIM_DELETE — иконка остаётся висеть, пока по ней не проведут мышью.
    # Обработчик меню выполняется в потоке трея, так что вызов синхронный.
    try:
        icon.visible = False
    except Exception as e:
        logging.error(f"Не удалось убрать значок из трея: {e}")
    icon.stop()
    os._exit(0)
